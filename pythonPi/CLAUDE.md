...
- **ML assist (AUTO mode):** `MlManager` runs the assist regression ONNX model in its own OS process (CPU-pinned, batch-of-1 at 20 Hz) over oar IMU windows. Each sample it queues `struct.pack('f?', prediction, paddleIdle)` to `MotorManager` (prediction is always the true model output; the idle flag travels beside it). Mapping:
...
- **Paddle idle gate:** while the paddle is still, AUTO targets 0 RPM (no deadband floor); the ramp-down tau shapes the spin-down. Detection = rolling 3 s std of oar gyro magnitude with enter/exit hysteresis, state machine in `MlManager._updateIdleGate`. Parameters come from the ONNX meta sidecar's `idle_gate` block (single source of truth: `mlTraining/config.py`; reference implementation `mlTraining/idle_gate.py` — the streaming version is parity-tested against it sample-for-sample and must stay identical, including `ddof=1` in the std). Training drops paddle-idle samples, so the model has never seen a still paddle — never consult its output in that state. Missing `idle_gate` block (old export) disables the gate with a warning. Both `assist_model` and `motor_cmd` Influx points carry `paddle_idle` (and `assist_model` carries `paddle_motion_energy`) for threshold tuning; Grafana panes exist in `grafana/dashboards/`.
